...
EasyProfilerCuda::EasyProfilerCuda(const std::string& prof_label, CUstream_st* stream, int event_cache_size, bool enabled, bool debug, std::ostream& ost)
: enabled(enabled),
  debug(debug),
...
EasyProfilerCuda::~EasyProfilerCuda() {
  ...
  for (int i = 1; i < labels.size(); i++) {
    ...
    ost << label << ": CPU=" << msec_cpu << "[msec] (accum=" << msec_cpu_accum << "[msec]) GPU=" << msec_gpu << "[msec] (accum=" << msec_gpu_accum << "[msec])" << '\n';
  }
